...
const Home = ({ posts}: Props) => {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <Head>
        <title>Medium Clone by Chocoscoding</title>
       ...
       {posts.map(post => (
         <Link key={post._id} href={`/post/${post.slug.current}`}>
           <div className='group cursor-pointer border rounded-lg overflow-hidden'>
             <img className='h-60 w-full object-cover group-hover:scale-105 transition-transform duration-200 ease-in-out' src={urlFor(post.mainImage).url()!} alt={post.slug.current} />
             <div className='flex justify-between p-5 bg-white'>
               <div>
                 <p className='texr-lg font-bold'>{post.title}</p>
